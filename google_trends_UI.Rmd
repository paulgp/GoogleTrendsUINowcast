...
weightedhits_aroundpartial <- policy_search_lf %>%
  group_by(daystopartial) %>%
  summarize_at("hits", list(weighted.mean), weight = "lf_022020") %>%
  filter(daystopartial >= -7 & daystopartial <= 7) %>%
  mutate(day = daystopartial) %>%
  rename(hits_partial = hits)

weightedhits_aroundcomplete <- policy_search_lf %>%
  group_by(daystocomplete) %>%
  summarize_at("hits", list(weighted.mean), weight = "lf_022020") %>%
  filter(daystocomplete >= -7 & daystocomplete <= 7) %>%
  mutate(day = daystocomplete) %>%
  rename(hits_complete = hits)

weighthits_aroundpolicies <- left_join(weightedhits_aroundcomplete, weightedhits_aroundpartial, by = "day")

#Plot (first individually, then together)
ggplot(weightedhits_aroundpartial, aes(x = daystopartial, y = hits_partial)) + 
  geom_line() + 
  geom_vline(aes(xintercept = 0, show.guide = TRUE), alpha = .3, size = 1.5, color = "mediumpurple2") +
...
ggplot(weightedhits_aroundcomplete, aes(x = daystocomplete, y = hits_complete)) + 
  geom_line() + 
  geom_vline(aes(xintercept = 0, show.guide = TRUE), alpha = .3, size = 1.5, color = "cadetblue3") +
  theme_classic(base_size=10) + 
  labs(x = "Days from Complete Closure",
       y = "Google Search Intensity", 
       title = "Average Search Intensity Around Time of Complete Closure",
       subtitle = "Weighted by size of states' labor force in Feb. 2020") +
  scale_x_continuous(breaks = seq(-7, 7, by = 1))

ggplot(weighthits_aroundpolicies) + 
  geom_line(aes(x = day, y = hits_complete), color = "cadetblue3") + 
  geom_line(aes(x = day, y = hits_partial), color = "mediumpurple2") + 
  geom_vline(aes(xintercept = 0, show.guide = TRUE), alpha = .3, size = 1.5, color = "black") +
  theme_classic(base_size=10) + 
  labs(x = "Days from Complete Closure",
       y = "Google Search Intensity", 
       title = "Average Search Intensity Around Time of Complete Closure",
...
#Set these to whatever you want!
  plotstate = "NY" #Change this to see another state
...
#Reshape to wide
weekly_claims_policy_wide1 <- subset(weekly_claims_policy, select = c("state", "week", "claims"))
weekly_claims_policy_wide2 <- weekly_claims_policy %>% 
    spread(week, claims) %>%
    rename("week13" = "13") %>%
    rename("week12" = "12") %>%
    rename("week11" = "11") %>%
...
weekly_claims_policy_wide2 <- weekly_claims_policy_wide2 %>% 
...
#Set up shape files
states <- map_data("state")
claims_chg_map_data <- left_join(states, weekly_claims_policy_wide2, by = "region")
