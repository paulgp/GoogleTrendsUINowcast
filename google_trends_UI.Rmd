...
#   if (i < 49) {
#     panel_data[[i]] = pull_data(loc = location_vec$location[i:(i+3)], time_window="2020-2-01 2020-3-26", panel=TRUE)
#   }
#   else {
#      panel_data[[i]] = pull_data(loc = location_vec$location[i:(i+2)], time_window="2020-2-01 2020-3-26", panel=TRUE)
#   }
#     # be polite
#     Sys.sleep(.2)
# }
...
# data_states_short %>% write_csv("data/data_states_2020_02_01_2020_03_30b.csv")
data_states_short = read_csv("data/data_states_2020_02_01_2020_03_30b.csv") 
```
...
weekly_predict_data = daily_predict_data %>% 
  ...
  mutate(predicted_ui = case_when(week == 12 ~ predicted_ui,
                                               TRUE ~ predicted_ui * (7/as.numeric(days))),
         predicted_ui_ub = case_when(week == 12 ~ predicted_ui_ub,
                                               TRUE ~ predicted_ui_ub * (7/as.numeric(days))), 
          predicted_ui_lb = case_when(week == 12 ~ predicted_ui_lb,
                                               TRUE ~ predicted_ui_lb * (7/as.numeric(days))))
...
output = weekly_predict_data %>% 
  mutate(week_str = case_when(week == 12 ~ "3/15-3/21",
                              week == 13 ~ "3/22-3/28")) %>%
    group_by(week_str) %>%
  select(week_str, predicted_ui, predicted_ui_lb, predicted_ui_ub) %>%
  summarize_all(sum) %>% mutate(predicted_ui = predicted_ui/ 1000000, predicted_ui_lb = predicted_ui_lb   / 1000000, predicted_ui_ub = predicted_ui_ub / 1000000)
...
options(knitr.kable.NA = "--")
weekly_predict_data %>% ungroup() %>% filter(week == 12) %>%
  select(-week) %>%
  ...
  kable(digits = 0, format.args = list(big.mark = ",", scientific = FALSE), 
         col.names = c("State", "UI Claims From News", "Google Trends Change", "Advance Official UI Claims", "Predicted UI Claims", "Predicted UI Claims")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F) %>%
  add_header_above(c("Week 3/15-3/21" = 4, "Daily Model" = 1, "Weekly Model" = 1)) %>%
...
a = summary(lm(ui_claims_daily/baseline_ui ~ hits_norm, 
           data =  daily_plot_data %>%
             ### COMMENT THIS OUT
         filter(epiweek(date) == 12),
           na.action = na.omit), robust=TRUE)
a
b = summary(lm(ui_claims_daily/baseline_ui ~ hits_norm, 
           data =  daily_plot_data %>% 
             ### COMMENT THIS OUT
         filter(epiweek(date) == 12), weight = baseline_ui,
           na.action = na.omit), robust=TRUE)
...
We find a strong relationship between the two variables, with an adjusted $R^{2}$ of `r round(a$adj.r.squared, digits = 4)` with an unweighted bivariate regression, and an adjusted $R^{2}$ of `r round(b$adj.r.squared, digits = 4)` when we weight observations by the states' baseline UI claims. In our results below, we focus on the estimates from the weighted regression. We use this estimated model and observed Google Trends changes to predict unemployment claims for the states lacking news-based estimates.
...
In this case, we predict `r round(output$predicted_ui[1], digits = 2)` million UI claims, with a 95\% CI of `r round(output$predicted_ui_lb[1], digits = 2)` million and `r round(output$predicted_ui_ub[1], digits = 2)` million for the week of 3/15-3/21.

For the week of 3/22-3/28, we need to extrapolate Google Trends interest for the rest of the week. The figure of Google Trends interest above shows that Sunday, March 22, the first day that will be included in the report, was 500 percent higher than the prior Sunday and Monday 250 percent higher than the prior Monday. Combining this information with our model and conservatively assuming that the search interest will remain flat at its current level through Saturday 3/28, implies a prediction of `r round(output$predicted_ui[2], digits = 2)` million UI claims, with a 95\% CI of `r round(output$predicted_ui_lb[2], digits = 2)` million and `r round(output$predicted_ui_ub[2], digits = 2)` million for the week of 3/22-3/28.


```{r, echo=FALSE, warning=FALSE, message=FALSE}

output  %>%  ungroup() %>% select(`Week` = week_str, `Predicted UI Claims (millions)` = predicted_ui, 
                                  `Lower Bound`=predicted_ui_lb,
                                  `Upper Bound`=predicted_ui_ub) %>% 
...
weekly_model = summary(lm(ui_growth ~ diff, data =joined_data, weight=baseline_ui, na.action= na.exclude ), robust=TRUE)
summary(lm(ui_growth ~ diff, data =joined_data, weight=baseline_ui, na.action= na.exclude ), robust=TRUE)

```

The model has an adjusted $R^2$ of `r round(weekly_model$adj.r.squared, digits = 4)`. We next use this estimated model and observed Google Trends changes to predict unemployment claims for the 15 states lacking news-based estimates.
...
Based on the advance reports from the states, we can evaluate how we did in our forecasts.
...
ggplot(data = evaluation) +
  geom_text(aes(x = Advance/1000, y = `Predicted UI Claims`/1000, label=State, color = "Daily Model")) +
  geom_text(aes(x = Advance/1000, y = `Forecasted UI Claims`/1000, label=State, color = "Weekly Model")) +
