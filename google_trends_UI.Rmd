...
report_level_UI_data <- read_excel("data/report_level_UI_data.xlsx", 
                                                        sheet = "zs") %>%
...
report_level_UI_data <- report_level_UI_data %>% bind_rows(read_excel("data/report_level_UI_data.xlsx", 
                                                        sheet = "dp") %>%
  mutate(date_start = date(date_start),
         date_end = date(date_end),
         article_date = janitor::excel_numeric_to_date(as.numeric(article_date))
...
The second part of our approach imputes state's UI claims harnessing data from Google Trends. We construct a daily panel dataset of the intensity of search for the term "file for unemployment" across state  We regress this measure on the set of states where we have constructed a growth rate in UI claims using news reports, and use this to impute the initial claims for all states. We do this by forecasting UI claims  using the estimated daily model and the panel of Google trends data.

These models are new and only partially validated.  We always welcome feedback (asojourn@umn.edu).

# Summary of Results

For the week ending March 21, the model yields initial national UI claims (seasonally adjusted)   of `r round(report_output_text$predicted_ui_sa[1], digits = 2)`. To put this in context,  `r round(report_output_text$predicted_ui_sa[1], digits = 1)` million Americans moving from employment to unemployment would imply a 2.7 percentage point increase in the unemployment rate in a single week, jumping by more than half from 3.5 percent to over 6 percent. Our confidence intervals range from `r round(report_output_text$predicted_ui_lb_sa[1], digits = 2)`	to `r round(report_output_text$predicted_ui_ub_sa[1], digits = 2)` million. Relative to the advance estimates from the state, this number is high. Further below, we discuss reasons for this, mainly related to seemingly low UI numbers from states which had had news reports suggesting much higher numbers. This discrepency is likely due to overwhelmed UI offices in these states. 

For the week of 3/22-3/28, we use the estimated relationship from 3/15-3/21 between Google Trends interest and UI claims to predict claims for the rest of the week. Google Trends interest shows that Sunday, March 22, the first day that will be included in the report, was 500 percent higher than the prior Sunday and Monday 250 percent higher than the prior Monday. Search interest continued to be extremely high throughout the week. Combining this information with our model implies a prediction of `r round(report_output_text$predicted_ui_sa[2], digits = 2)` million UI claims, seasonally adjusted, with a 95\% CI of `r round(report_output_text$predicted_ui_lb_sa[2], digits = 2)` million and `r round(report_output_text$predicted_ui_ub_sa[2], digits = 2)` million for the week of 3/22-3/28.
...
report_output %>% 
  kable(digits = 0, format.args = list(big.mark = ",", scientific = FALSE), 
         col.names = c("State", "UI Claims From News", "Advance Official UI Claims", "Google Trends Change from 2/22-3/14",  "Predicted UI Claims", "Google Trends Change from 2/22-3/14",  "Predicted UI Claims", "Predicted UI Claims")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F) %>%
  add_header_above(c(" " = 1, "Week 3/15-3/21" = 4, "Week 3/22-3/28" = 2, "Combined Weeks 3/15-3/28")) %>%
...
For the week of 3/15-3/21, we predicted `r round(report_output_text$predicted_ui_sa[1], digits = 2)` million UI claims, with a 95\% CI of `r round(report_output_text$predicted_ui_lb_sa[1], digits = 2)` million and `r round(report_output_text$predicted_ui_ub_sa[1], digits = 2)` million for the week of 3/15-3/21.

For the week of 3/22-3/28, the figure of Google Trends interest above shows that Sunday, March 22, the first day that will be included in the report, was 500 percent higher than the prior Sunday and Monday 250 percent higher than the prior Monday. Google Trends interest continues to be high throughout the week, suggesting that there is significant UI activity. Combining this information with our model implies a prediction of `r round(report_output_text$predicted_ui_sa[2], digits = 2)` million UI claims, with a 95\% CI of `r round(report_output_text$predicted_ui_lb_sa[2], digits = 2)` million and `r round(report_output_text$predicted_ui_ub_sa[2], digits = 2)` million for the week of 3/22-3/28. 
...
For the week of 3/15-3/21, there are initial advance reports of UI claims at the state level that reported on March 26. We compare our predictions for that week relative to the advance numbers, and find that for many states we did quite well. However, for certain states, particularly California and New York, we were substantially over. This discrepency appears to be not a fault of the model, but rather a breakdown of the UI system in those states -- many states report being overwhelmed by applications, and these states (including CA and NY) only initially reported numbers that were officially in the system.
...
Since we only use the week of 3/15-3/21 to estimate the model, we can also use the news reports from 3/22-3/28 to assess how well we are doing. This is our leave-out sample, and hence will give us a sense of how stable the relationship is. 
...
We do remarkably well, with an adjusted $R^{2}$ of `r round(b$adj.r.squared, digits = 2)`.

<!-- ### Slopegraph over time (temporary) -->
<!-- ```{r} -->
<!-- MySpecial <- list(   -->
...
<!--     theme(axis.text.y      = element_text(text(size=14))), -->
...
<!-- ) -->
<!-- regions <- read_excel("data/regions.xlsx") -->
<!-- plt_data = weekly_predict_data %>% bind_rows(UI_Claims_March21 %>% select(location, predicted_ui =  baseline_ui) %>% mutate(week = 11))  %>% left_join(regions) %>% mutate(predicted_ui = round(predicted_ui / 1000,digits=1)) -->
<!-- ggplot(data = plt_data, aes(x = week, y = predicted_ui, group = location))  + -->
<!--   geom_line(aes(color = location, alpha = 1), size = 1) +  -->
<!--   geom_text_repel(data = plt_data %>% filter(week == 11),  -->
<!--                   aes(label = location) ,  -->
<!--                   hjust = "left",  -->
<!--                   fontface = "bold",  -->
<!--                   size = 3,  -->
<!--                   nudge_x = -40,  -->
<!--                   direction = "y") + -->
<!--     geom_text_repel(data = plt_data %>% filter(week == 13),  -->
<!--                   aes(label = location) ,  -->
<!--                   hjust = "right",  -->
<!--                   fontface = "bold",  -->
<!--                   size = 3,  -->
<!--                   nudge_x = 40,  -->
<!--                   direction = "y") + -->
<!--     geom_label(aes(label = predicted_ui),  -->
<!--              size = 2.5,  -->
<!--              label.padding = unit(0.05, "lines"),  -->
<!--              label.size = 0.0) + -->
<!--   facet_wrap(~bigregion, nrow=4)  +  -->
<!--   scale_x_continuous(breaks = c(11,12,13), -->
<!--                      labels = c("2/22-3/14" = 11,  -->
<!--                               "3/15-3/21" = 12,  -->
<!--                               "3/22-3/28" = 13)) + -->
<!--   MySpecial + -->
<!--   scale_y_log10() -->

<!-- plt_data_subset = plt_data %>% inner_join(UI_Claims_March21 %>% arrange(baseline_ui) %>% filter(row_number() > 2*n()/3)) -->
<!-- ggplot(data = plt_data_subset, aes(x = week, y = predicted_ui, group = location))  + -->
<!--   geom_line(aes(color = location, alpha = 1), size = 1) +  -->
<!--   geom_text_repel(data = plt_data_subset %>% filter(week == 11),  -->
<!--                   aes(label = location) ,  -->
<!--                   hjust = "left",  -->
<!--                   fontface = "bold",  -->
<!--                   size = 3,  -->
<!--                   nudge_x = -40,  -->
<!--                   direction = "y") + -->
<!--     geom_text_repel(data = plt_data_subset %>% filter(week == 13),  -->
<!--                   aes(label = location) ,  -->
<!--                   hjust = "right",  -->
<!--                   fontface = "bold",  -->
<!--                   size = 3,  -->
